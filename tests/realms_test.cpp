...
struct harness {
	...
	static araya::desired_component node(std::string path, araya::component_spec s) {
...
TEST_CASE("isolating a key hides the ancestor binding and scopes its own") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		auto root = rt.root();
		root->bind(db_key.id, araya::binding{std::make_shared<database>(database{"root"}), 7});
		...
		isolated->bind(db_key.id, araya::binding{std::make_shared<database>(database{"test"}), 8});
		auto* iso = isolated->lookup(db_key.id);
		REQUIRE(iso != nullptr);
		CHECK(static_cast<database*>(iso->value.get())->name == "test");
		CHECK(static_cast<database*>(root->lookup(db_key.id)->value.get())->name == "root");

		// A sibling without the tag is unaffected.
		auto sibling = root->make_child();
		CHECK(static_cast<database*>(sibling->lookup(db_key.id)->value.get())->name == "root");

		// Keys sharing a realm share one binding slot.
		auto shared = root->make_child();
		shared->isolate(db_key.id, "pool");
		shared->bind(db_key.id, araya::binding{std::make_shared<database>(database{"pool"}), 9});
		CHECK(shared->lookup(db_key.id) == shared->lookup(db_key.id));
		co_return;
	});
}
...
TEST_CASE("reconcile isolates a provider and its consumer into one realm") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		CHECK(std::find(g_log.begin(), g_log.end(), "consume:p1") != g_log.end());
	});
}
...
TEST_CASE("realm reassignment moves the binding without reloading the "
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		co_await rt.reconcile(std::move(desired));
		co_await rt.wait_idle();
		REQUIRE(std::find(g_log.begin(), g_log.end(), "consume:p1") != g_log.end());
...
TEST_CASE("removing the isolate annotation restores plain resolution") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		desired.clear();
		desired.push_back(h.node("p", h.spec(&g_provider_desc, {{"value", "p1"}})));
		desired.push_back(h.node("c", h.spec(&g_consumer_desc)));
		co_await rt.reconcile(std::move(desired));
		co_await rt.wait_idle();
