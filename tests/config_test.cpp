...
araya::plugin_config cfg(std::initializer_list<araya::plugin_config::value_type> v) { return araya::plugin_config(v); }

} // namespace

TEST_CASE("arithmetic config values parse through from_chars") {
	auto c = cfg({{"int", "12"}, {"ll", "-42"}, {"double", "3.25"}, {"u8", "200"}, {"bool", "true"}});
...
TEST_CASE("bool accepts the four documented forms") {
	for (auto const& [text, want] : std::initializer_list<std::pair<std::string, bool>>{
...
TEST_CASE("plugin-specific parser specializations are honored") {
	...
	CHECK_THROWS_AS(araya::plugin_config_view(bad)[word_key], araya::config_error);
}
...
TEST_CASE("parse_value serves non-plugin_config inputs") {
	...
	CHECK(araya::parse_value<long long>("-7") == std::optional<long long>(-7));
