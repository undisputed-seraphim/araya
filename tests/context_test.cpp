...
TEST_CASE("root context resolves a bound service") {
    ...
    auto lease =
        araya::service_lease<database>(std::static_pointer_cast<database>(
                                             b->value),
                                         b->provider);
...
TEST_CASE("service identity includes name and version") {
    ...
    root->bind(araya::service_id{"example.database", 1},
               araya::binding{v1, 1});
    root->bind(araya::service_id{"example.database", 2},
               araya::binding{v2, 2});

    CHECK(root->lookup(araya::service_id{"example.database", 1})->value ==
          v1);
    CHECK(root->lookup(araya::service_id{"example.database", 2})->value ==
          v2);
