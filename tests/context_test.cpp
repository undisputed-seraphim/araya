...
inline constexpr araya::service_key<database> db_key{"example.database", 1};
inline constexpr araya::service_key<cache> cache_key{"example.cache", 1};
...
TEST_CASE("root context resolves a bound service") {
	...
	root->bind(db_key.id, araya::binding{db, 7});

	auto const* b = root->lookup(db_key.id);
	REQUIRE(b != nullptr);
	CHECK(b->provider == 7);

	auto lease = araya::service_lease<database>(std::static_pointer_cast<database>(b->value), b->provider);
...
TEST_CASE("child scope overrides parent binding without mutating parent") {
	...
	root->bind(db_key.id, araya::binding{production, 1});
	child->bind(db_key.id, araya::binding{test, 2});
...
TEST_CASE("child scope falls back to parent bindings") {
	...
	auto db = std::make_shared<database>("production");
	root->bind(db_key.id, araya::binding{db, 1});
...
TEST_CASE("service identity includes name and version") {
	...
	root->bind(araya::service_id{"example.database", 1}, araya::binding{v1, 1});
	root->bind(araya::service_id{"example.database", 2}, araya::binding{v2, 2});

	CHECK(root->lookup(araya::service_id{"example.database", 1})->value == v1);
	CHECK(root->lookup(araya::service_id{"example.database", 2})->value == v2);
	CHECK(root->lookup(araya::service_id{"example.database", 3}) == nullptr);
	CHECK(root->lookup(araya::service_id{"other.database", 1}) == nullptr);
}
...
TEST_CASE("plugin_context provides and requires services") {
	...
	CHECK_THROWS_AS(ctx.require<database>(db_key), araya::resolution_error);
}
...
TEST_CASE("resolution_error carries service identity") {
	...
	} catch (araya::resolution_error const& e) {
...
TEST_CASE("withdrawing one provider does not remove another's binding") {
	auto root = araya::context::root();
	auto act_a = std::make_shared<araya::activation>(root);
	auto act_b = std::make_shared<araya::activation>(root);
	araya::plugin_context ctx_a{act_a};
	araya::plugin_context ctx_b{act_b};
