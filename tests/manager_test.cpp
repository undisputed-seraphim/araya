...
struct provider_plugin : araya::plugin {
	...
	araya::task<void> apply(araya::plugin_context& ctx) override {
		ctx.effect([tag = "provide:" + value] { return log_effect(tag); });
...
struct consumer_plugin : araya::plugin {
	...
	araya::task<void> apply(araya::plugin_context& ctx) override {
		...
		ctx.effect([tag = tag, shared = db.shared()] {
			...
			return [tag, shared] { g_log.push_back("c-cleanup:" + tag + ":" + shared->name); };
		});
		co_return;
	}
};
...
std::unique_ptr<araya::plugin> make_slow_provider(araya::plugin_config const&) {
	return std::make_unique<slow_provider_plugin>();
}

struct broken_plugin : araya::plugin {
	araya::task<void> apply(araya::plugin_context& ctx) override {
		ctx.effect([tag = std::string("broken")] { return log_effect(tag); });
...
std::unique_ptr<araya::plugin> make_undeclared(araya::plugin_config const&) {
...
static const araya::plugin_descriptor g_slow_provider_desc{"slow-provider", g_no_deps, g_db_prov, &make_slow_provider};
static const araya::plugin_descriptor g_broken_desc{"broken", g_no_deps, g_no_provs, &make_broken};
static const araya::plugin_descriptor g_undeclared_desc{"undeclared", g_no_deps, g_no_provs, &make_undeclared};
...
TEST_CASE("consumer waits for its dependencies") {
	harness h;
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		auto consumer = co_await rt.mount(h.spec(&g_consumer_desc, {{"tag", "c1"}}));
		co_await rt.wait_idle();
	...
	CHECK(g_log == std::vector<std::string>{"setup:provide:p1", "active:p1", "consume:c1:p1", "c-setup:c1"});
}

TEST_CASE("retiring a provider unloads consumers before provider cleanup") {
	harness h;
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		auto provider = co_await rt.mount(h.spec(&g_provider_desc, {{"value", "p1"}}));
		auto consumer = co_await rt.mount(h.spec(&g_consumer_desc, {{"tag", "c1"}}));
		co_await rt.wait_idle();
	...
	auto c_cleanup = std::find(g_log.begin(), g_log.end(), "c-cleanup:c1:p1");
	auto p_cleanup = std::find(g_log.begin(), g_log.end(), "cleanup:provide:p1");
...
TEST_CASE("provider replacement reactivates consumers") {
	...
	CHECK(std::find(g_log.begin(), g_log.end(), "consume:c1:p2") != g_log.end());
	CHECK(std::find(g_log.begin(), g_log.end(), "consume:c1:p1") != g_log.end());
}
...
TEST_CASE("consumers mounted while a provider is retiring stay inactive") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		auto late = co_await rt.mount(h.spec(&g_consumer_desc, {{"tag", "late"}}));
	...
	CHECK(std::find(g_log.begin(), g_log.end(), "consume:late:p2") != g_log.end());
}
...
TEST_CASE("optional dependencies allow activation without a provider") {
	harness h;
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		auto consumer = co_await rt.mount(h.spec(&g_optional_consumer_desc));
		co_await rt.wait_idle();
		CHECK(rt.state_of(consumer.id()) == araya::fiber_state::active);
		CHECK(rt.error_of(consumer.id()) == nullptr);
...
TEST_CASE("plugin failure cleans partial effects and stays inactive") {
	...
	CHECK(g_log == std::vector<std::string>{"setup:broken", "cleanup:broken"});
}
...
TEST_CASE("root context provides services to consumers") {
	...
	CHECK(std::find(g_log.begin(), g_log.end(), "consume:c1:rootdb") != g_log.end());
}
...
TEST_CASE("two consumers drain before their shared provider unloads") {
	...
	auto c1_cleanup = std::find(g_log.begin(), g_log.end(), "c-cleanup:a:p1");
	auto c2_cleanup = std::find(g_log.begin(), g_log.end(), "c-cleanup:b:p1");
	auto p_cleanup = std::find(g_log.begin(), g_log.end(), "cleanup:provide:p1");
