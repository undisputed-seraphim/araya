...
struct harness {
    ...
    void run(Fn&& fn) {
        ...
        g_child_desc = const_cast<medulla::plugin_descriptor*>(&g_child_desc_v);
        boost::asio::co_spawn(
            io.get_executor(),
...
TEST_CASE("a plugin mounts a child that reads through the parent's view") {
    ...
    CHECK(std::find(g_log.begin(), g_log.end(), "parent-active:p1") !=
          g_log.end());
    CHECK(std::find(g_log.begin(), g_log.end(), "child-active:a:p1") !=
          g_log.end());
}
...
TEST_CASE("retiring a parent cascades to its children") {
    ...
    auto child_cleanup = std::find(g_log.begin(), g_log.end(),
                                   "child-cleanup:a");
    auto parent_cleanup = std::find(g_log.begin(), g_log.end(),
                                    "parent-cleanup:a");
...
TEST_CASE("provider retirement cascades through parents to children") {
    ...
    auto parent_cleanup = std::find(g_log.begin(), g_log.end(),
                                    "parent-cleanup:a");
    auto provider_cleanup = std::find(g_log.begin(), g_log.end(),
                                      "p-cleanup:p:p1");
    REQUIRE(child_cleanup != g_log.end());
    REQUIRE(parent_cleanup != g_log.end());
