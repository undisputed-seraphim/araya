...
struct seq_driver {
    ...
    medulla::task<void> operator()() {
        ...
        for (auto const& [s, h] : slot)
            out->fibers[s] = {rt->state_of(h.id()),
                              rt->error_of(h.id()) != nullptr};
...
template <class Driver>
void run_once(boost::asio::io_context& io, Driver d,
              observable& result) {
    auto holder = std::make_shared<Driver>(std::move(d));
    auto out = holder->out;
    boost::asio::co_spawn(io.get_executor(),
                          ref_driver<Driver>{std::move(holder)},
                          boost::asio::detached);
    io.run();
    io.restart();
    result = *out;
...
TEST_CASE("all short operation sequences quiesce, validate, and are "
    ...
    SUCCEED("enumerated " << all.size()
                          << " sequences over "
                          << by_config.size() << " configurations");
}
