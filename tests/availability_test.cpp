...
std::unique_ptr<araya::plugin> make_provider(
    ...
    if (auto it = cfg.find("start"); it != cfg.end() &&
                                       it->second == "ready")
        p->ready = true;
    return p;
}
...
struct throwing_plugin : araya::plugin {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ctx.provide(db_key, std::make_shared<database>("p"),
                    []() -> bool { throw std::runtime_error("nope"); });
        co_return;
    }
};

std::unique_ptr<araya::plugin> make_throwing(
...
std::unique_ptr<araya::plugin> make_deactivator(
    araya::plugin_config const&) {
...
struct counting_plugin : araya::plugin {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        ctx.on(promote_key, [ctx](std::string const&) {
            ctx.set_available(db_key, true);
        });
        co_return;
    }
...
static const araya::plugin_descriptor g_desc_P{"P", g_no_deps, g_db_prov,
                                               &make_provider};
static const araya::plugin_descriptor g_desc_T{"T", g_no_deps, g_db_prov,
                                               &make_throwing};
static const araya::plugin_descriptor g_desc_N{"N", g_no_deps, g_db_prov,
                                               &make_counting};
static const araya::plugin_descriptor g_desc_C{"C", g_db_dep, g_no_provs,
                                               &make_consumer};
static const araya::plugin_descriptor g_desc_O{"O", g_db_opt, g_no_provs,
                                               &make_optional};
static const araya::plugin_descriptor g_desc_I{"I", g_db_dep, g_no_provs,
                                               &make_impostor};
static const araya::plugin_descriptor g_desc_D{"D", g_no_deps, g_db_prov,
                                               &make_deactivator};
