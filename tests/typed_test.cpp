...
std::unique_ptr<araya::plugin> make_typed_provider(araya::plugin_config const&) {
...
struct typed_consumer : araya::plugin {
	araya::task<void> apply(araya::plugin_context& ctx) override {
		...
		static_assert(std::is_same_v<decltype(db), araya::tagged_lease<database, araya::committed_tag>>);
		co_return;
	}
};

std::unique_ptr<araya::plugin> make_typed_consumer(araya::plugin_config const&) {
...
TEST_CASE("typestate handles consume their transitions") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		auto active = araya::typed_handle<araya::fiber_state::active>::claim(raw);
		co_await rt.wait_idle();
