...
araya::task<void> wait_until_stopped(bool& seen) {
	...
	while (!token.stop_requested()) {
		auto timer = boost::asio::steady_timer(co_await boost::asio::this_coro::executor, 10ms);
...
TEST_CASE("spawned task completes and fiber becomes active") {
	...
	int result = 0;
	auto h = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<int> {
							  result = 42;
							  co_return 42;
						  }));
...
TEST_CASE("exception marks fiber inactive") {
	boost::asio::io_context io;
	auto h = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> {
...
TEST_CASE("spawned clean completion publishes a null error") {
	boost::asio::io_context io;
	auto h = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> { co_return; }));
...
TEST_CASE("handle error is null while the task still runs") {
	boost::asio::io_context io;
	auto h = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> {
							  auto timer = boost::asio::steady_timer(co_await boost::asio::this_coro::executor, 40ms);
							  co_await timer.async_wait(boost::asio::use_awaitable);
						  }));
...
TEST_CASE("cancel is observed cooperatively through this_stop_token") {
	...
	cancel_timer.async_wait([h](boost::system::error_code) { h.cancel(); });
...
TEST_CASE("cancel does not abort an in-flight operation") {
	...
	auto h = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> {
							  auto timer = boost::asio::steady_timer(co_await boost::asio::this_coro::executor, 60ms);
							  co_await timer.async_wait(boost::asio::use_awaitable);
							  timer_done = true;
							  auto token = co_await araya::this_stop_token();
...
TEST_CASE("stop_requested returns immediately outside a fiber") {
	...
	boost::asio::co_spawn(
		io.get_executor(),
		araya_test::heap_coroutine([&]() -> araya::task<void> { value = co_await araya::stop_requested(); }),
		boost::asio::detached);
...
TEST_CASE("cancelling one fiber does not disturb another") {
	...
	auto first = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> {
								  co_await wait_until_stopped(first_seen);
							  }));

	auto second = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> {
								   second_done = true;
								   co_return;
							   }));

	boost::asio::steady_timer cancel_timer{io, 50ms};
	cancel_timer.async_wait([first](boost::system::error_code) { first.cancel(); });
...
TEST_CASE("fiber ids are unique and increasing") {
	boost::asio::io_context io;
	auto a = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> { co_return; }));
	auto b = araya::spawn(io.get_executor(), araya_test::heap_coroutine([&]() -> araya::task<void> { co_return; }));
