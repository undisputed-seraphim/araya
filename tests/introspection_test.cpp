...
std::unique_ptr<araya::plugin> make_provider(araya::plugin_config const&) {
...
static const araya::plugin_descriptor g_desc_B{"B", g_db_dep, g_no_provs, &make_broken};
...
std::unique_ptr<araya::plugin> make_spy(araya::plugin_config const&) { return std::make_unique<spy_plugin>(); }

static const araya::plugin_descriptor g_desc_spy{"spy", g_no_deps, g_no_provs, &make_spy};
...
TEST_CASE("plugin_context::root reaches the runtime root from any scope") {
	...
	run(io, [rt, child]() -> araya::task<void> {
		auto spec = araya::component_spec{shared_desc(g_desc_spy), {}, child, "spy", {}};
...
TEST_CASE("fibers() snapshots ids, states, declarations, and views") {
	...
	run(io, [&]() -> araya::task<void> {
		p = co_await rt->mount(araya::component_spec{shared_desc(g_desc_P), {}, nullptr, "p-instance", {}});
		c = co_await rt->mount(araya::component_spec{shared_desc(g_desc_C), {}, nullptr, "", {}});
		b = co_await rt->mount(araya::component_spec{shared_desc(g_desc_B), {}, nullptr, "b-instance", {}});
		co_await rt->wait_idle();
	});
	...
	CHECK(ci->committed.begin()->first == araya::owned_service_id{db_key.id});
...
TEST_CASE("fibers_async returns the snapshot from off-strand callers") {
	...
	run(io, [&]() -> araya::task<void> {
		co_await rt->mount(araya::component_spec{shared_desc(g_desc_P), {}, nullptr, "p", {}});
		co_await rt->wait_idle();
	});

	std::vector<araya::fiber_info> seen;
	run(io, [&]() -> araya::task<void> { seen = co_await rt->fibers_async(); });
