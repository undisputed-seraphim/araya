...
static void mod_apply(araya_instance_v1* self, araya_fiber fiber,
    ...
    if (api->effect(
            ...
            &inst->tag, nullptr) != ARAYA_ABI_OK) {
        complete(fiber, ARAYA_ABI_ERROR, "effect failed");
        return;
    }

    if (api->on(fiber, "example.notify", 1, ARAYA_MODE_SERIAL,
...
static void slow_apply(araya_instance_v1*, araya_fiber fiber,
                       void (*complete)(araya_fiber, int, const char*)) {
...
static araya_instance_v1* make_slow(const araya_config_pair*,
                                      std::size_t) {
...
static void failing_apply(araya_instance_v1*, araya_fiber fiber,
                          void (*complete)(araya_fiber, int,
                                           const char*)) {
    complete(fiber, ARAYA_ABI_ERROR, "module failed on purpose");
}

static araya_instance_v1* make_failing(const araya_config_pair*,
                                         std::size_t) {
...
static araya_plugin_descriptor_v1 g_mod_desc{
    ARAYA_ABI_VERSION, "module", g_inject, 1, g_provide, 1, &make_mod};
static araya_plugin_descriptor_v1 g_slow_desc{
    ARAYA_ABI_VERSION, "slow-module", nullptr, 0, nullptr, 0, &make_slow};
static araya_plugin_descriptor_v1 g_failing_desc{
    ARAYA_ABI_VERSION, "failing-module", nullptr, 0, nullptr, 0,
    &make_failing};

const araya_plugin_descriptor_v1* make_failing_entry(
    const araya_host_api_v1*) {
    return &g_failing_desc;
}

const araya_plugin_descriptor_v1* make_slow_entry(
...
TEST_CASE("module failure fails the activation") {
    ...
    h.run([&](araya::runtime& rt) -> araya::task<void> {
        ...
        auto fh = co_await rt.mount(
            araya::component_spec{desc, {}, nullptr, ""});
        co_await rt.wait_idle();
...
TEST_CASE("module without reconfigure is replaced on config change") {
    ...
    h.run([&](araya::runtime& rt) -> araya::task<void> {
        ...
        desired.push_back(araya::desired_component{
            "mod",
            araya::component_spec{desc, {{"tag", "r1"}}, nullptr, ""}});
        co_await rt.reconcile(std::move(desired));
        co_await rt.wait_idle();

        std::vector<araya::desired_component> updated;
        updated.push_back(araya::desired_component{
            "mod",
            araya::component_spec{desc, {{"tag", "r2"}}, nullptr, ""}});
