...
bool marker_exists() { return std::filesystem::exists("/tmp/araya_module_unloaded.marker"); }

void remove_marker() { std::filesystem::remove("/tmp/araya_module_unloaded.marker"); }
...
TEST_CASE("module loader mounts and runs a dlopen'ed module") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		auto fh = co_await rt.mount(araya::component_spec{desc, {{"tag", "so1"}}, nullptr, ""});
...
TEST_CASE("module unloads after its last descriptor reference dies") {
	remove_marker();
	{
		...
		h.run([&](araya::runtime& rt) -> araya::task<void> {
			...
			auto fh = co_await rt.mount(araya::component_spec{desc, {{"tag", "so2"}}, nullptr, ""});
			co_await rt.wait_idle();
			CHECK(rt.state_of(fh.id()) == araya::fiber_state::active);
...
TEST_CASE("module stays loaded while fibers still reference it") {
	...
	std::shared_ptr<araya::plugin_descriptor> held;
	{
		...
		h.run([&](araya::runtime& rt) -> araya::task<void> {
			...
			auto fh = co_await rt.mount(araya::component_spec{held, {{"tag", "so3"}}, nullptr, ""});
			co_await rt.wait_idle();
			CHECK(rt.state_of(fh.id()) == araya::fiber_state::active);
			co_await rt.retire(fh);
			co_await rt.wait_idle();
...
TEST_CASE("module loader reports missing files") {
	araya::module_loader loader;
	CHECK_THROWS_AS(loader.load("/nonexistent/definitely/missing.so"), std::runtime_error);
}
