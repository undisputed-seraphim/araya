...
araya::registration record(std::vector<std::string>& log, std::string tag, araya::plugin_context& ctx) {
...
TEST_CASE("cleanup runs in reverse registration order") {
	...
	act->teardown();
	CHECK(log == std::vector<std::string>{"setup:1", "setup:2", "setup:3", "cleanup:3", "cleanup:2", "cleanup:1"});
}
...
TEST_CASE("early release runs the cleanup immediately") {
	...
	act->teardown();
	CHECK(log == std::vector<std::string>{"setup:1", "setup:2", "setup:3", "cleanup:2", "cleanup:3", "cleanup:1"});
}
...
TEST_CASE("throwing setup records nothing and keeps earlier effects") {
	...
	record(log, "1", ctx);
	CHECK_THROWS_AS(
		ctx.effect([]() -> araya::cleanup_action { throw std::runtime_error("setup failed"); }), std::runtime_error);
...
TEST_CASE("throwing cleanup does not stop remaining teardown") {
	...
	CHECK(log == std::vector<std::string>{"setup:1", "setup:3", "cleanup:3", "cleanup:bad", "cleanup:1"});
}
