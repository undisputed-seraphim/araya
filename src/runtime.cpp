...
boost::asio::awaitable<void> runtime::run_on_strand(std::move_only_function<void()> fn) {
	co_await boost::asio::post(strand_, boost::asio::use_awaitable);
	fn();
}

boost::asio::awaitable<void> runtime::reconcile(std::vector<desired_component> desired) {
	co_await boost::asio::post(strand_, boost::asio::use_awaitable);
	...
	std::vector<std::pair<fiber_id, std::shared_ptr<detail::gate_impl>>> to_await;
	...
	for (auto it = reconciled_.begin(); it != reconciled_.end();) {
		...
		bool same_impl = f->spec.descriptor && f->spec.descriptor->name == node->spec.descriptor->name;
		if (!same_impl) {
			f->reactivate = -1;
			to_await.emplace_back(f->id, f->done);
			begin_unload(*f, false);
		...
		if (f->spec.config != node->spec.config) {
			bool applied =
				f->instance && f->state == fiber_state::active && f->instance->reconfigure(node->spec.config);
			if (!applied) {
				f->reactivate = -1;
				to_await.emplace_back(f->id, f->done);
				begin_unload(*f, false);
		...
		std::map<owned_service_id, std::string, transparent_id_less> old_realms;
		...
		if (scope_moved || realms_changed) {
			if (f->state == fiber_state::active || f->state == fiber_state::loading) {
...
std::shared_ptr<context> runtime::entry_scope_for(std::string const& path, component_spec const& spec) {
...
std::size_t runtime::fiber_count() const noexcept { return fibers_.size(); }
...
void runtime::on_diagnostic(std::move_only_function<void(diagnostic const&)> sink) {
...
runtime::resolution runtime::resolve(fiber_record const& f) const {
	...
	for (auto const& [key, required] : f.inject) {
		...
		if (b && b->value && b->state == provider_state::active && b->available && b->provider != f.id) {
			r.providers[key] = b->provider;
			r.bindings[key] = *b;
		} else if (required) {
			r.satisfiable = false;
			r.error = std::make_exception_ptr(resolution_error{service_id{key.name, key.version}});
			return r;
		}
	}
	return r;
...
void runtime::start_loading(fiber_record& f) {
	...
	if (!f.instance) {
		f.error = std::make_exception_ptr(
			std::logic_error("plugin factory returned null for '" + std::string(f.spec.descriptor->name) + "'"));
		f.state = fiber_state::inactive;
		f.control->cell->publish_error(f.error);
		f.control->cell->state->store(fiber_state::inactive);
		f.activation.reset();
	...
	boost::asio::co_spawn(
		...
		boost::asio::bind_cancellation_slot(
			f.control->cell->signal->slot(), [this, id](std::exception_ptr ep) { on_apply_completed(id, ep); }));
}

void runtime::on_apply_completed(fiber_id id, std::exception_ptr ep) {
	auto* f = find(id);
	if (!f || (f->state != fiber_state::loading && f->state != fiber_state::unloading))
		return;
	detail::retire_fiber(f->strand);

	bool stale = !providers_still_active(*f);
	bool cancelled = f->control->cell->stop_source->stop_requested();
	...
	if (ep || cancelled || stale || f->state == fiber_state::unloading) {
		if (!f->error) {
			if (ep) {
				...
				f->error = std::make_exception_ptr(std::runtime_error("fiber cancelled before activation completed"));
			}
		}
		if (f->activation) {
...
bool runtime::providers_still_active(fiber_record const& f) const {
	for (auto const& [key, p] : f.committed) {
		auto const* b = f.spec.parent->lookup(key);
		if (!b || !b->value || b->provider != p || b->state != provider_state::active || !b->available)
			return false;
	}
	return true;
}
...
void runtime::signal_availability(service_id key, context* scope, std::uint64_t provider, bool available) {
	...
	if (b->provider != provider)
		throw std::logic_error(
			"signal_availability: fiber " + std::to_string(provider) + " does not provide '" + std::string(key.name) +
			"'");
	if (!available)
		throw std::logic_error(
...
void runtime::publish(fiber_record& f) {
	...
	for (auto const& key : f.provide) {
		if (auto* b = f.spec.parent->lookup_mutable(key); b && b->provider == f.id) {
...
void runtime::finish_unload(fiber_record& f) {
	...
	for (auto const& [k, p] : committed) {
		auto it = fibers_.find(p);
		if (it != fibers_.end()) {
			[[maybe_unused]] auto erased = it->second->consumers.erase(f.id);
...
void runtime::reassign(
	fiber_record& f,
	component_spec const& next,
	std::shared_ptr<context> const& new_scope,
	std::map<owned_service_id, std::string, transparent_id_less> const& old_realms) {
...
void runtime::diagnose(bool scan_cycles) {
	auto emit =
		[this](diagnostic_kind kind, std::string key_name, std::uint32_t key_version, std::vector<fiber_id> ids) {
	...
	for (auto const& [id, f] : fibers_)
		for (auto const& k : f->provide)
			diag_slots_.emplace_back(diag_slot_key{f->spec.parent.get(), k, f->spec.parent->realm_for(k)}, id);
	std::sort(diag_slots_.begin(), diag_slots_.end(), [](auto const& a, auto const& b) { return a.first < b.first; });
	for (std::size_t i = 0; i < diag_slots_.size();) {
		std::size_t j = i + 1;
		while (j < diag_slots_.size() && !(diag_slots_[i].first < diag_slots_[j].first) &&
		...
		if (j - i >= 2) {
			...
			std::string key_name = slot.realm.empty() ? std::string(slot.key.name) : slot.realm + "@" + slot.key.name;
			emit(diagnostic_kind::conflict, std::move(key_name), slot.key.version, std::move(ids));
		}
		i = j;
	}
	...
	auto pos = [&](fiber_id id) {
		return static_cast<int>(std::lower_bound(diag_order_.begin(), diag_order_.end(), id) - diag_order_.begin());
	};
	...
	if (scan_cycles) {
		...
		for (auto const& [mid, m] : fibers_) {
			...
			diag_targets_.erase(std::unique(diag_targets_.begin(), diag_targets_.end()), diag_targets_.end());
	...
	for (auto const& [id, f] : fibers_) {
		for (auto const& k : f->provide)
			if (std::find(f->inject_keys.begin(), f->inject_keys.end(), k) != f->inject_keys.end()) {
...
void runtime::validate_invariants() const {
	for (auto const& [id, f] : fibers_) {
		...
		if (f->state == fiber_state::inactive) {
			...
			ARAYA_ASSERT(f->remaining == 0);
			ARAYA_ASSERT(
				f->activation == nullptr ||
				(f->activation->state == fiber_state::inactive && f->activation->effects->size() == 0));
			continue;
		}
		...
		for (auto const& [k, p] : f->committed) {
			ARAYA_ASSERT(std::find(f->inject_keys.begin(), f->inject_keys.end(), k) != f->inject_keys.end());
			[[maybe_unused]] auto pit = fibers_.find(p);
			ARAYA_ASSERT(pit != fibers_.end());
			ARAYA_ASSERT(pit->second->state != fiber_state::inactive);
		}
		if (f->activation) {
			ARAYA_ASSERT(f->activation->committed_view.size() == f->committed.size());
