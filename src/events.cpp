...
event_bus::event_bus(boost::asio::strand<boost::asio::any_io_executor> control_strand)
	: strand_(std::move(control_strand)) {}

void event_bus::ensure_on_strand() const {
	if (!strand_.running_in_this_thread())
		throw std::logic_error("event bus operations must run on the control strand");
}

detail::event_entry_base* event_bus::entry_base_for(service_id id) noexcept {
...
std::uint64_t event_bus::add_raw_listener(
	...
	if (mode == dispatch_mode::bail)
		throw std::logic_error(
			"raw listeners cannot register on bail-mode "
			"event '" +
			std::string(id.name) + "'");
	auto* base = entry_base_for(id);
	if (!base)
		throw std::logic_error("raw listener for undeclared event '" + std::string(id.name) + "'");
	if (base->mode() != mode)
		throw std::logic_error("listener mode mismatch for event '" + std::string(id.name) + "'");
...
void event_bus::set_diagnostic_sink(std::move_only_function<void(std::exception_ptr)> sink) {
	*sink_ = std::move(sink);
}
