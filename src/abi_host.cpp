...
struct abi_bridge {
	explicit abi_bridge(plugin_context c)
		: ctx(std::move(c)) {}
	plugin_context ctx;
...
static abi_bridge* fiber_bridge(araya_fiber fiber) { return static_cast<abi_bridge*>(fiber); }

static int host_require(araya_fiber fiber, const char* name, std::uint32_t version, void** out) {
	try {
		auto b = fiber_bridge(fiber)->ctx.find_binding(service_id{name, version});
		if (!b)
...
static int host_find(araya_fiber fiber, const char* name, std::uint32_t version, void** out) {
	try {
		auto b = fiber_bridge(fiber)->ctx.find_binding(service_id{name, version});
...
static int host_provide(
	...
	try {
		...
		std::shared_ptr<void> owned(value, destroy_value ? destroy_value : +[](void*) {});
		auto reg = std::make_shared<registration>(b->ctx.provide_raw(service_id{name, version}, std::move(owned)));
...
static int
host_effect(araya_fiber fiber, void (*setup)(void* data, araya_cleanup_v1* cleanup), void* data, araya_handle* out) {
	try {
		auto* b = fiber_bridge(fiber);
		auto reg = std::make_shared<registration>(b->ctx.effect([setup, data]() -> cleanup_action {
			araya_cleanup_v1 c{};
			if (setup)
				setup(data, &c);
			if (!c.run)
...
static int host_on(
	...
	std::uint32_t version,
	std::uint32_t mode,
	void (*invoke)(void* data, const void* msg),
	void* data,
	araya_handle* out) {
	try {
		auto* b = fiber_bridge(fiber);
		...
		auto token = bus->add_raw_listener(
			id,
			static_cast<dispatch_mode>(mode),
			...
			b->act->id);
		auto index =
			b->act->effects->add([bus, key = owned_service_id(id), token] { bus->remove_listener(key, token); });
...
static void host_post(araya_fiber fiber, void (*fn)(void* data), void* data) {
...
araya_host_api_v1 const& host_api() noexcept { return g_host_api; }

namespace {

class abi_plugin final : public plugin {
public:
	explicit abi_plugin(araya_instance_v1* instance)
		: instance_(instance) {}
	...
	bool reconfigure(plugin_config const& cfg) override {
		...
		return instance_->reconfigure(instance_, pairs.data(), pairs.size()) != 0;
	}

private:
	static void on_complete(araya_fiber fiber, int code, const char* error) {
		auto* b = static_cast<abi_bridge*>(fiber);
		if (code != ARAYA_ABI_OK) {
			b->error = std::make_exception_ptr(std::runtime_error(error ? error : "module activation failed"));
		}
		b->done->open();
	}
...
std::shared_ptr<plugin_descriptor> wrap(araya_plugin_entry_fn entry, std::shared_ptr<void> keep_alive) {
	if (!entry)
	...
	for (std::size_t i = 0; i < c->inject_count; ++i) {
		...
		holder->deps.push_back(dependency_spec{service_id{holder->names[1 + i], dep.version}, dep.required != 0, {}});
	}
	for (std::size_t i = 0; i < c->provide_count; ++i) {
		auto const& prov = c->provide[i];
		holder->provs.push_back(provision_spec{service_id{holder->names[1 + c->inject_count + i], prov.version}});
	}
	holder->desc.inject = holder->deps;
	holder->desc.provide = holder->provs;

	holder->desc.create = [c, keep_alive = std::move(keep_alive)](plugin_config const& cfg) -> std::unique_ptr<plugin> {
		std::vector<araya_config_pair> pairs;
		pairs.reserve(cfg.size());
		for (auto const& [k, v] : cfg)
			pairs.push_back({k.c_str(), v.c_str()});
