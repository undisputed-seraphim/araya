...
context::context(std::shared_ptr<context> parent, bool pass_through_bind)
    : parent_(std::move(parent)), pass_through_bind_(pass_through_bind) {}
...
void context::unisolate(service_id id) {
    realms_.erase(id);
}
...
void context::bind(service_id id, binding b) {
    bind_realm(id, realm_for(id), std::move(b));
}
...
void context::unbind(service_id id) noexcept {
    unbind_realm(id, realm_for(id));
}

binding const* context::lookup_realm(service_id id,
...
binding const* context::lookup(service_id id) const noexcept {
    return lookup_realm(id, realm_for(id));
}
