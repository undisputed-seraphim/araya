...
std::shared_ptr<plugin_descriptor> module_loader::load(std::string const& path) {
	void* handle = dlopen(path.c_str(), RTLD_NOW | RTLD_LOCAL);
	if (!handle) {
		const char* error = dlerror();
		throw std::runtime_error("dlopen failed for '" + path + "': " + (error ? error : "unknown error"));
	}

	auto module = std::make_shared<module_impl>(handle);

	auto entry = reinterpret_cast<araya_plugin_entry_fn>(dlsym(handle, "araya_plugin_entry_v1"));
	if (!entry)
		throw std::runtime_error("module '" + path + "' does not export araya_plugin_entry_v1");
