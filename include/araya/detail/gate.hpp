...
struct gate_impl : std::enable_shared_from_this<gate_impl> {
	...
	auto wait(Token&& token) {
		return boost::asio::async_initiate<Token, void()>(wait_op{shared_from_this()}, token);
	}
...
inline std::shared_ptr<gate_impl> make_gate(bool open = false) { return std::make_shared<gate_impl>(open); }
