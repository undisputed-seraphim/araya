...
public:
	...
	boost::asio::awaitable<void> reconcile(std::vector<desired_component> desired);

	// Resumes on the control strand and invokes fn there.
	boost::asio::awaitable<void> run_on_strand(std::move_only_function<void()> fn);
	...
	void on_diagnostic(std::move_only_function<void(diagnostic const&)> sink);
...
private:
	...
	std::shared_ptr<context> entry_scope_for(std::string const& path, component_spec const& spec);
	...
	std::map<owned_service_id, std::set<fiber_id>, transparent_id_less> consumers_of_;
	...
	std::map<owned_service_id, std::set<fiber_id>, transparent_id_less> providers_of_;
	std::map<std::string, fiber_id> reconciled_;
	std::map<std::string, std::shared_ptr<context>> entry_ctxs_;
	std::map<std::string, std::map<std::string, std::string>> entry_isolates_;
	...
	struct diag_slot_key {
		...
		friend bool operator<(diag_slot_key const& a, diag_slot_key const& b) noexcept {
