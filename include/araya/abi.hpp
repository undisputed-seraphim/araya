...
struct araya_instance_v1 {
	...
	void (*apply)(
		araya_instance_v1* self,
		araya_fiber fiber,
		void (*complete)(araya_fiber fiber, int code, const char* error));

	// Optional in-place reconfiguration. Returns nonzero on success.
	// May be null.
	int (*reconfigure)(araya_instance_v1* self, const araya_config_pair* pairs, std::size_t count);
...
struct araya_host_api_v1 {
	...
	int (*require)(araya_fiber fiber, const char* name, std::uint32_t version, void** out);

	// Like require, but missing services are not an error.
	int (*find)(araya_fiber fiber, const char* name, std::uint32_t version, void** out);
...
struct araya_plugin_descriptor_v1 {
	...
	araya_instance_v1* (*create)(const araya_config_pair* pairs, std::size_t count);
};

using araya_plugin_entry_fn = const araya_plugin_descriptor_v1* (*)(const araya_host_api_v1*);

// Every native module exports this symbol.
const araya_plugin_descriptor_v1* araya_plugin_entry_v1(const araya_host_api_v1* host);
