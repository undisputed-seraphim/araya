...
public:
	config_error(std::string_view key, std::string_view value, std::string_view reason)
		: std::runtime_error(
			  "config key '" + std::string(key) + "' value '" + std::string(value) + "': " + std::string(reason))
		, key_(key)
		, value_(value)
		, reason_(reason) {}
...
struct config_key {
	...
	constexpr config_key(std::string_view n) noexcept
		: name(n) {}
};
...
struct config_parser<std::string_view> {
	static std::optional<std::string_view> parse(std::string_view text) { return text; }
};
...
public:
	...
	bool contains(std::string_view key) const noexcept { return cfg_->find(std::string(key)) != cfg_->end(); }
...
private:
	plugin_config::const_iterator find(std::string_view key) const { return cfg_->find(std::string(key)); }
