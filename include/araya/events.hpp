...
struct event_key {
	...
	constexpr event_key(std::string_view name, std::uint32_t version = 1) noexcept
		: id{name, version} {}

	constexpr event_key(service_id i) noexcept
		: id(i) {}
};
...
struct waterfall_slot {
	...
	std::function<boost::asio::awaitable<Message>(Message const&, waterfall_continuation<Message>)> fn;
...
auto make_notify_listener(Fn fn) {
	return [fn = std::move(fn)](Message const& m) -> boost::asio::awaitable<void> {
...
auto make_waterfall_listener(Fn fn) {
	return [fn = std::move(fn)](
			   Message const& m, waterfall_continuation<Message> next) -> boost::asio::awaitable<Message> {
		using result_t = std::invoke_result_t<Fn const&, Message const&, waterfall_continuation<Message>>;
...
auto make_bail_listener(Fn fn) {
	return [fn = std::move(fn)](Message const& m) -> boost::asio::awaitable<bool> {
		using result_t = std::invoke_result_t<Fn const&, Message const&>;
...
class waterfall_continuation {
public:
	waterfall_continuation(std::shared_ptr<std::vector<detail::waterfall_slot<Message>>> listeners, std::size_t index)
	...
	boost::asio::awaitable<Message> operator()(Message msg) {
		if (index_ >= listeners_->size())
			co_return msg;
		co_return co_await (*listeners_)[index_].fn(msg, waterfall_continuation{listeners_, index_ + 1});
	}
...
struct event_entry_base {
	...
	virtual void
	add_raw(std::uint64_t token, std::uint64_t owner, std::function<boost::asio::awaitable<void>(void const*)> fn) = 0;
};
...
struct parallel_state {
	explicit parallel_state(std::size_t total, boost::asio::any_io_executor strand)
		: remaining(total)
		, gate(make_gate(*this, std::move(strand))) {}
...
private:
	static boost::asio::experimental::promise<void()>
	make_gate(parallel_state& self, boost::asio::any_io_executor strand) {
		boost::asio::experimental::use_promise_t<> token;
		return boost::asio::async_initiate<boost::asio::experimental::use_promise_t<>, void()>(
			boost::asio::bind_executor(strand, [&self](auto&& handler) { self.complete_handler = handler; }), token);
	}
};
...
public:
	...
	std::size_t count() const noexcept override { return listeners_->size() + raw_->size(); }
	...
	void add_raw(std::uint64_t token, std::uint64_t owner, std::function<boost::asio::awaitable<void>(void const*)> fn)
		override {
	...
	bool deliver(slot_t const& slot, service_id id, context const* scope) const noexcept {
	...
	dispatch_parallel(Message const& msg, boost::asio::any_io_executor strand, service_id id, context const* scope) {
		auto snapshot = listeners_;
		auto raw_snapshot = raw_;
		...
		for (auto const& slot : *snapshot) {
			if (!deliver(slot, id, scope))
				continue;
			auto t = slot.fn(msg);
			boost::asio::co_spawn(strand, std::move(t), [state](std::exception_ptr ep) { state->complete(ep); });
		}
		for (auto const& slot : *raw_snapshot) {
			auto t = slot.fn(&msg);
			boost::asio::co_spawn(strand, std::move(t), [state](std::exception_ptr ep) { state->complete(ep); });
	...
	boost::asio::awaitable<void> dispatch_serial(Message const& msg, service_id id, context const* scope) {
		auto snapshot = listeners_;
		for (auto const& slot : *snapshot) {
			if (!deliver(slot, id, scope))
				continue;
	...
	boost::asio::awaitable<Message> dispatch_waterfall(Message const& msg, service_id id, context const* scope) {
		if (!scope) {
			auto snapshot = listeners_;
	...
	boost::asio::awaitable<bool> dispatch_bail(Message const& msg, service_id id, context const* scope) {
		auto snapshot = listeners_;
		for (auto const& slot : *snapshot) {
			if (!deliver(slot, id, scope))
				continue;
...
private:
	...
	std::shared_ptr<std::vector<raw_slot<Message>>> raw_ = std::make_shared<std::vector<raw_slot<Message>>>();
};
...
public:
	...
	bool on_control_strand() const noexcept { return strand_.running_in_this_thread(); }

	void set_diagnostic_sink(std::move_only_function<void(std::exception_ptr)> sink);

	void report(std::exception_ptr ep);

	template <class Message, dispatch_mode Mode, class Fn>
	std::uint64_t
	add_listener(event_key<Message, Mode> const& key, Fn&& fn, std::uint64_t owner, listener_options opts = {}) {
		ensure_on_strand();
		auto* base = entry_base_for(key.id);
		if (!base) {
			entries_.insert_or_assign(
				owned_service_id(key.id), std::make_unique<detail::event_entry_impl<Message, Mode>>());
			base = entry_base_for(key.id);
		} else if (base->mode() != Mode) {
			throw std::logic_error(
				"listener registered with conflicting "
				"dispatch mode for event '" +
				std::string(key.id.name) + "'");
		}
		auto* impl = static_cast<detail::event_entry_impl<Message, Mode>*>(base);
		using slot_t = typename detail::event_entry_impl<Message, Mode>::slot_t;
		auto token = next_token_++;
		std::decay_t<decltype(slot_t::fn)> listener = detail::make_listener<Message, Mode>(std::forward<Fn>(fn));
		if (opts.once) {
			...
			listener = [listener = std::move(listener), remover = [this, id = owned_service_id(key.id), token]() {
				remove_listener(id, token);
			}]<class... Args>(Args&&... args) mutable -> decltype(auto) {
				remover();
				return listener(std::forward<Args>(args)...);
			};
		}
		impl->add(slot_t{owner, token, std::move(listener), opts}, opts.prepend);
		return token;
	}
	...
	template <class Message>
	void dispatch(event_key<Message, dispatch_mode::emit> const& key, Message msg, context const* scope = nullptr) {
		ensure_on_strand();
	...
	template <class Message>
	boost::asio::awaitable<void>
	dispatch(event_key<Message, dispatch_mode::serial> const& key, Message const& msg, context const* scope = nullptr) {
		ensure_on_strand();
	...
	dispatch(event_key<Message, dispatch_mode::bail> const& key, Message const& msg, context const* scope = nullptr) {
		ensure_on_strand();
...
private:
	...
	detail::event_entry_impl<Message, Mode>* typed_entry(service_id id) {
		...
		if (!base)
			return nullptr;
		if (base->mode() != Mode)
			throw std::logic_error("dispatch mode mismatch for event '" + std::string(id.name) + "'");
	...
	std::map<owned_service_id, std::unique_ptr<detail::event_entry_base>, transparent_id_less> entries_;
