...
template <typename Handler>
struct stop_wait_state : stop_wait_state_base,
                         std::enable_shared_from_this<stop_wait_state<Handler>> {
    Handler handler;
    boost::asio::any_io_executor executor;
    std::stop_token token;
    std::optional<std::stop_callback<stop_wait_thunk<Handler>>> callback;

    stop_wait_state(Handler h, boost::asio::any_io_executor ex,
                    std::stop_token tok)
        : handler(std::move(h)), executor(std::move(ex)), token(std::move(tok)) {
    }

    void arm() {
        callback.emplace(token,
                         stop_wait_thunk<Handler>{this->shared_from_this()});
    }
...
auto stop_wait(std::stop_token token, CompletionToken&& t) {
    return boost::asio::async_initiate<CompletionToken, void()>(
        stop_wait_initiation{std::move(token)}, t);
}
