...
public:
    ...
    void reconcile_all(
        std::map<int, std::pair<char, std::string>> const& desired);
...
private:
    ...
    component const& comp(fiber const& f) const {
        return pool_.at(f.comp);
    }
    ...
    bool satisfiable(fiber const& f,
                     std::map<std::string, int>& providers) const {
        for (auto const& [key, required] : comp(f).inject) {
            auto const* b = lookup(key);
            if (b && b->provider != -1 && b->state == pstate::active &&
                b->provider != f.fidx) {
    ...
    bool providers_still_active(fiber const& f) const {
        for (auto const& [key, p] : f.committed) {
            auto const* b = lookup(key);
            if (!b || b->provider == -1 || b->provider != p ||
                b->state != pstate::active)
                return false;
        }
        return true;
    }
...
inline void machine::reconcile_all(
    std::map<int, std::pair<char, std::string>> const& desired) {
    ...
    for (auto const& [slot, wanted] : desired) {
        ...
        if (f && declared) {
            ...
            bool keep = same_impl && !same_cfg &&
                        f->state == fstate::active &&
                        pool_.at(comp).reconfigures;
...
inline void machine::start_loading(fiber& f,
                                   std::map<std::string, int> providers) {
...
inline void machine::begin_unload(fiber& f, bool flag) {
    ...
    for (auto c : consumers)
        if (auto* consumer = at(c); consumer &&
                                     consumer->state != fstate::inactive)
            ++f.remaining;
    for (auto c : consumers)
        if (auto* consumer = at(c); consumer)
