...
struct logged : araya::plugin {
    ...
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        (void)ctx.effect([]() -> araya::cleanup_action {
            return [] { g_log.push_back("cleanup:" + name); };
        });
        co_return;
    }
};
...
struct provider_plugin : logged<'P'> {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        (void)ctx.effect([]() -> araya::cleanup_action {
            return [] { g_log.push_back("cleanup:P"); };
        });
        co_return;
    }
};
...
struct reconfiguring_provider_plugin : logged<'Q'> {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        (void)ctx.effect([]() -> araya::cleanup_action {
            return [] { g_log.push_back("cleanup:Q"); };
        });
        co_return;
    }
...
struct consumer_plugin : logged<'C'> {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        (void)ctx.effect([]() -> araya::cleanup_action {
            return [] { g_log.push_back("cleanup:C"); };
        });
        co_return;
    }
};
...
struct optional_plugin : logged<'O'> {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        (void)ctx.effect([]() -> araya::cleanup_action {
            return [] { g_log.push_back("cleanup:O"); };
        });
        co_return;
    }
};
...
struct self_plugin : logged<'S'> {
    araya::task<void> apply(araya::plugin_context& ctx) override {
        ...
        (void)ctx.effect([]() -> araya::cleanup_action {
            return [] { g_log.push_back("cleanup:S"); };
        });
        co_return;
    }
};
...
static const araya::plugin_descriptor g_desc_Q{
    "Q", g_no_deps, g_db_prov, &make<reconfiguring_provider_plugin>};

araya::plugin_descriptor const* descriptor_for(char c) {
    switch (c) {
        case 'P': return &g_desc_P;
        case 'C': return &g_desc_C;
        case 'O': return &g_desc_O;
        case 'S': return &g_desc_S;
        case 'B': return &g_desc_B;
        case 'Q': return &g_desc_Q;
    }
    return nullptr;
}
    ...
    araya::task<void> operator()() {
        ...
        for (auto const& o : seq) {
            if (!op_valid(o, sv[o.slot]))
                continue;
            if (o.kind == 'M') {
                auto h = co_await rt->mount(araya::component_spec{
                    shared_desc(o.component),
                    {{"v", std::string(1, o.cfg)}}, nullptr, ""});
                slot[o.slot] = h;
...
observable run_engine(std::vector<op> const& seq) {
    ...
    boost::asio::co_spawn(io.get_executor(), holder{d},
                          boost::asio::detached);
    io.run();
    return *out;
}
