...
araya::registration timer_service::timeout(plugin_context& caller, duration d, std::move_only_function<void()> fn) {
	auto timer = std::make_shared<boost::asio::steady_timer>(executor_);
	timer->expires_after(d);
	timer->async_wait([timer, fn = std::move(fn)](boost::system::error_code ec) mutable {
		if (!ec)
			fn();
	});
	return caller.effect([timer]() -> araya::cleanup_action { return [timer] { timer->cancel(); }; });
}

araya::registration timer_service::interval(plugin_context& caller, duration d, std::move_only_function<bool()> fn) {
	...
	state->body = std::make_shared<std::move_only_function<bool()>>(std::move(fn));
	state->d = d;
	state->schedule();
	return caller.effect([timer = state->timer]() -> araya::cleanup_action { return [timer] { timer->cancel(); }; });
}
