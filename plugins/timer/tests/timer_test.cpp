...
std::unique_ptr<araya::plugin> make_timer_owner(araya::plugin_config const&) {
...
static const araya::dependency_spec g_timer_dep[]{{araya::service_id{"timer", 1}, true}};
static const araya::plugin_descriptor g_timer_owner_desc{"timer-owner", g_timer_dep, g_no_provs, &make_timer_owner};
...
TEST_CASE("sleep awaits at least the requested delay") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		auto service = root_ctx.require<timer_service>(timer_key).shared();
