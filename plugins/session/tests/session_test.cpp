...
boost::json::value message(std::string id, std::string role, boost::json::value content) {
	return boost::json::object{{"id", std::move(id)}, {"role", std::move(role)}, {"content", std::move(content)}};
}

boost::json::value assistant(std::string id, boost::json::value content) {
	return boost::json::object{{"message", message(std::move(id), "assistant", std::move(content))}};
}

boost::json::value system(std::string id, std::string plugin, boost::json::value content) {
	boost::json::object m = message(std::move(id), "system", std::move(content)).as_object();
	m["source"] = boost::json::object{{"kind", "plugin"}, {"plugin", std::move(plugin)}};
...
boost::json::value tool_result(std::string id, std::string call_id) {
	...
	content.emplace_back(boost::json::object{{"tool_call_id", call_id}, {"type", "tool_result"}});
	return boost::json::object{
		{"id", std::move(id)},
		{"role", "user"},
		{"content", std::move(content)},
		{"source", boost::json::object{{"kind", "tool"}, {"call_id", std::move(call_id)}}}};
}

struct logger_plugin : araya::plugin {
	araya::task<void> apply(araya::plugin_context& ctx) override {
		ctx.on(created_key, [](session_created_msg const& m) { g_log.push_back("created:" + m.s->id().value); });
		ctx.on(appended_key, [](session_appended_msg const& m) { g_log.push_back("ev:" + m.event.type); });
		ctx.on(disposed_key, [](session_disposed_msg const& m) { g_log.push_back("disposed:" + m.id.value); });
...
std::unique_ptr<araya::plugin> make_logger(araya::plugin_config const&) { return std::make_unique<logger_plugin>(); }
...
std::unique_ptr<araya::plugin> make_owner(araya::plugin_config const&) { return std::make_unique<owner_plugin>(); }
...
struct projection_plugin : araya::plugin {
	araya::task<void> apply(araya::plugin_context& ctx) override {
		...
		(void)store->register_message_projection(
			ctx, {"custom/counter", [](session_event const& ev) -> std::optional<session_message> {
					  return session_message{
						  message_role::user,
						  "custom-" + std::string(ev.data.at("count").as_string()),
						  ev.data,
						  std::nullopt,
						  std::nullopt};
				  }});
...
std::unique_ptr<araya::plugin> make_projection(araya::plugin_config const&) {
...
static const araya::dependency_spec g_sessions_dep[]{{araya::service_id{"sessions", 1}, true}};
static const araya::plugin_descriptor g_logger_desc{"logger", g_sessions_dep, g_no_provs, &make_logger};
static const araya::plugin_descriptor g_owner_desc{"owner", g_sessions_dep, g_no_provs, &make_owner};
static const araya::plugin_descriptor g_projection_desc{"projection", g_sessions_dep, g_no_provs, &make_projection};
...
TEST_CASE("create announces, dispose removes, and the firehose sees every "
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		s->append("user/message", message("m1", "user", boost::json::array{}));
		s->append("assistant/message", assistant("m2", boost::json::array{}));
		co_await s->flush();
		...
		CHECK(std::find(g_log.begin(), g_log.end(), "created:s1") != g_log.end());
		CHECK(std::find(g_log.begin(), g_log.end(), "ev:user/message") != g_log.end());
		CHECK(std::find(g_log.begin(), g_log.end(), "ev:assistant/message") != g_log.end());
		CHECK(std::find(g_log.begin(), g_log.end(), "flush:s1") != g_log.end());
		CHECK(std::find(g_log.begin(), g_log.end(), "disposed:s1") != g_log.end());
	});
}
...
TEST_CASE("the surface folds the built-in message types in order") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		auto store = h.store(root_ctx);
		auto s = store->create(root_ctx, session_id{"s1"});
		s->append("system/message", system("sys1", "prompts", boost::json::array{}));
		s->append("user/message", message("u1", "user", boost::json::array{}));
		s->append("assistant/message", assistant("a1", boost::json::array{}));
...
TEST_CASE("projections fold their type while their fiber lives") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		CHECK(std::find(g_log.begin(), g_log.end(), "ev:custom/counter") != g_log.end());
	});
}
...
TEST_CASE("seeded forks carry lineage, an end-seed marker, and a live "
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		seed.push_back(session_event{0, 1000, "user/message", message("m1", "user", boost::json::array{}), false});
		seed.push_back(session_event{1, 1001, "assistant/message", assistant("m2", boost::json::array{}), false});
		...
		auto root_ctx = rt.root_context();
		auto store = h.store(root_ctx);
		auto s = store->create(root_ctx, session_id{"s1"}, std::move(options));
		...
		s->append("user/message", message("m3", "user", boost::json::array{}));
		co_await rt.wait_idle();
		CHECK(std::find(g_log.begin(), g_log.end(), "ev:user/message") != g_log.end());
	});
}
...
TEST_CASE("reconstruction closes interrupted tool calls") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		content.emplace_back(
			boost::json::object{{"type", "tool_call"}, {"tool_call", boost::json::object{{"id", "tc1"}}}});
		std::vector<session_event> seed;
		seed.push_back(session_event{0, 1000, "assistant/message", assistant("m1", std::move(content)), false});
...
TEST_CASE("unloading the owning fiber disposes its session") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		auto store = h.store(root_ctx);
		CHECK(store->size() == 1);
		CHECK(std::find(g_log.begin(), g_log.end(), "created:owned") != g_log.end());

		co_await rt.retire(owner);
		co_await rt.wait_idle();

		CHECK(store->size() == 0);
		CHECK(std::find(g_log.begin(), g_log.end(), "disposed:owned") != g_log.end());
	});
}
...
TEST_CASE("built-in message shapes are validated on append") {
	...
	h.run([&](araya::runtime& rt) -> araya::task<void> {
		...
		CHECK_THROWS(s->append("assistant/message", message("m1", "assistant", boost::json::array{})));
		// tool/result content must carry the matching tool_call_id.
		boost::json::array content;
		content.emplace_back(boost::json::object{{"tool_call_id", "tc1"}, {"type", "tool_result"}});
		CHECK_THROWS(s->append(
			...
			boost::json::object{
				...
				{"role", "user"},
				{"content", std::move(content)},
				{"source", boost::json::object{{"kind", "tool"}, {"call_id", "tc2"}}}}));
