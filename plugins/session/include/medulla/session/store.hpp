...
public:
    ...
    session_log_offset inherited_event_count() const noexcept {
        return inherited_event_count_;
    }
    ...
    medulla::task<void> flush();

private:
    friend class session_store;

    session(session_header header, session_log_offset inherited_event_count,
            session_seq first_live_seq, std::vector<session_event> log,
            std::vector<message_projection> const& projections,
            std::weak_ptr<session_store> store);

    void fold(session_event const& ev,
              std::vector<message_projection> const& projections);
...
public:
    ...
    std::shared_ptr<session> create(plugin_context& caller,
                                    session_id id = {},
    ...
    std::shared_ptr<session> prepare(session_id id,
                                     create_session_options options = {});
...
private:
    ...
    std::shared_ptr<medulla::event_bus> bus_;
    std::map<session_id, std::shared_ptr<session>> store_;
    std::vector<message_projection> projections_;
    std::uint64_t counter_ = 0;
};

inline constexpr medulla::service_key<session_store> sessions_key{"sessions",
                                                                  1};
