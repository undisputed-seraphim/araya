...
void session::fold(session_event const& ev,
                   std::vector<message_projection> const& projections) {
...
session_seq session::append(std::string type, boost::json::value data) {
    session_event ev{next_seq_++, now_ms(), std::move(type),
                     std::move(data), false};
...
session_id session_store::mint_id() {
    return session_id{"session-" + std::to_string(counter_++)};
}

std::shared_ptr<session> session_store::create(
    plugin_context& caller, session_id id, create_session_options options) {
...
std::shared_ptr<session> session_store::prepare(
    session_id id, create_session_options options) {
    if (id.value.empty())
        throw std::invalid_argument("session id is empty");
    if (store_.contains(id))
        throw std::logic_error("session '" + id.value +
                               "' already exists");
    ...
    for (std::uint64_t i = 0; i < seed.size(); ++i) {
        if (seed[i].seq != i)
            throw std::invalid_argument(
                "seed seq discontinuity at index " + std::to_string(i));
        validate_event(seed[i]);
        seed[i].ignorable = !is_builtin_surface_type(seed[i].type) &&
                            !has_projection(seed[i].type);
    }
    ...
    if (options.is_seeded && options.inherited_event_count > 0) {
        seed.push_back(session_event{
            ...
            seed.empty() ? now_ms() : seed.back().time, "session/end-seed",
    ...
    return std::shared_ptr<session>(
        new session(std::move(h), options.inherited_event_count, seed_size,
                    std::move(seed), projections_, weak_from_this()));
}

void session_store::enter(std::shared_ptr<session> s) {
    if (store_.contains(s->id()))
        throw std::logic_error("session '" + s->id().value +
                               "' already exists");
...
void session_store::announce(session const& s) {
    ...
    if (it == store_.end())
        throw std::logic_error(
            "announce requires an entered session");
    if (bus_)
        ...
        return [weak, type] {
            if (auto st = weak.lock()) {
                std::erase_if(st->projections_,
                              [&type](message_projection const& p) {
...
boost::json::object const& message_object(session_event const& ev) {
    ...
    if (it == obj->end() || !it->value().is_object())
        bad("session event '" + ev.type +
            "' must wrap the message under \"message\"");
    return it->value().as_object();
}

void require_id(boost::json::object const& msg, std::string_view type) {
    auto it = msg.find("id");
    if (it == msg.end() || !it->value().is_string() ||
        it->value().as_string().empty())
        bad("session event '" + std::string(type) +
            "' message lacks a non-empty string id");
}

void require_content(boost::json::object const& msg,
                     std::string_view type) {
    auto it = msg.find("content");
    if (it == msg.end() || !it->value().is_array())
        bad("session event '" + std::string(type) +
            "' message lacks an array content");
}
...
void session_store::validate_event(session_event const& ev) const {
    ...
    if (ev.type == "system/message") {
        ...
        if (nit == src.end() || !nit->value().is_string() ||
            nit->value().as_string().empty())
        ...
        if (cit == src.end() || !cit->value().is_string() ||
            cit->value().as_string() != call_id)
