...
void repair_interrupted_turns(std::vector<session_event>& log) {
	...
	auto const last_time = log.empty() ? now_ms() : log.back().time;
	for (auto const& call_id : open) {
		boost::json::array content;
		content.emplace_back(
			boost::json::object{{"tool_call_id", call_id}, {"type", "tool_result"}, {"outcome", "unknown"}});
		boost::json::object data;
		data["id"] = "repair-" + call_id;
		data["role"] = "user";
		data["content"] = std::move(content);
		data["source"] = boost::json::object{{"kind", "tool"}, {"call_id", call_id}};
		log.push_back(
			session_event{static_cast<session_seq>(log.size()), last_time, "tool/result", std::move(data), false});
	}
}
