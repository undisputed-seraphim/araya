...
std::optional<session_message> fold_builtin(session_event const& ev) {
	if (ev.type == "user/message") {
		...
		if (!id || it == obj->end())
			return std::nullopt;
		return session_message{message_role::user, *std::move(id), it->value(), std::nullopt, std::nullopt};
	}
	if (ev.type == "assistant/message") {
		...
		if (!id || it == obj->end())
			return std::nullopt;
		return session_message{message_role::assistant, *std::move(id), it->value(), std::nullopt, std::nullopt};
	}
	if (ev.type == "system/message") {
		...
		return session_message{message_role::system, *std::move(id), it->value(), std::nullopt, std::move(plugin)};
	}
	if (ev.type == "tool/result") {
		...
		return session_message{
			message_role::tool_result, *std::move(id), it->value(), std::move(call_id), std::nullopt};
	}
	return std::nullopt;
}
