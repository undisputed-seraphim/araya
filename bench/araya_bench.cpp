...
struct result {
	...
	double per_op_ns() const { return ops ? wall_ms * 1e6 / static_cast<double>(ops) : 0.0; }
};
...
std::shared_ptr<plugin_descriptor> shared_desc(plugin_descriptor const& d) {
	return std::shared_ptr<plugin_descriptor>(const_cast<plugin_descriptor*>(&d), [](plugin_descriptor*) {});
}
...
std::unique_ptr<araya::plugin> make_consumer(plugin_config const&) { return std::make_unique<consumer_plugin>(); }

std::unique_ptr<araya::plugin> make_optional(plugin_config const&) { return std::make_unique<optional_plugin>(); }

std::unique_ptr<araya::plugin> make_self(plugin_config const&) { return std::make_unique<self_plugin>(); }
...
static const plugin_descriptor g_desc_P{"P", g_no_deps, g_db_prov, &make_provider};
static const plugin_descriptor g_desc_C{"C", g_db_dep, g_no_provs, &make_consumer};
static const plugin_descriptor g_desc_O{"O", g_db_opt, g_no_provs, &make_optional};
static const plugin_descriptor g_desc_S{"S", g_db_opt, g_db_prov, &make_self};
...
component_spec consumer_spec() { return {shared_desc(g_desc_C), {}, nullptr, "C", {}}; }
...
struct chain_plugin : araya::plugin {
	chain_plugin(chain_set const* s, int l)
		: set(s)
		, level(l) {}
	chain_set const* set;
	int level;
...
struct head_plugin : araya::plugin {
	explicit head_plugin(chain_set const* s)
		: set(s) {}
	chain_set const* set;
...
struct tail_plugin : araya::plugin {
	explicit tail_plugin(chain_set const* s)
		: set(s) {}
	chain_set const* set;
...
struct chain_set {
	explicit chain_set(int depth)
		...
		for (int i = 0; i < depth; ++i) {
			deps[i] = araya::dependency_spec{araya::service_id{names[i + 1], 1}, true, {}};
			provs[i] = araya::provision_spec{araya::service_id{names[i], 1}};
			descs[i] = plugin_descriptor{
				...
				[this, i](plugin_config const&) { return std::make_unique<chain_plugin>(this, i); }};
		}
		head_dep = araya::dependency_spec{araya::service_id{names[0], 1}, true, {}};
		tail_prov = araya::provision_spec{araya::service_id{names[depth], 1}};
		head = plugin_descriptor{
			"head", std::span<araya::dependency_spec const>(&head_dep, 1), g_no_provs, [this](plugin_config const&) {
				return std::make_unique<head_plugin>(this);
			}};
		tail = plugin_descriptor{
			"tail", g_no_deps, std::span<araya::provision_spec const>(&tail_prov, 1), [this](plugin_config const&) {
...
task<void> chain_plugin::apply(araya::plugin_context& ctx) {
	...
	ctx.provide(set->key(level), std::make_shared<token>(set->names[level]));
	co_return;
}
...
task<void> tail_plugin::apply(araya::plugin_context& ctx) {
	ctx.provide(set->key(set->depth), std::make_shared<token>(set->names[set->depth]));
	co_return;
}
...
struct ticker_plugin : araya::plugin {
	task<void> apply(araya::plugin_context& ctx) override {
		...
		auto log = ctx.require<araya::logger::logger_service>(araya::logger::logger_key);
		auto timer = ctx.require<araya::timer::timer_service>(araya::timer::timer_key);
		...
		timer->interval(ctx, std::chrono::milliseconds(10), [logger]() mutable {
			logger.debug("tick {}", g_ticks.fetch_add(1, std::memory_order_relaxed) + 1);
			return true;
		});
		co_return;
	}
...
std::unique_ptr<araya::plugin> make_ticker(plugin_config const&) { return std::make_unique<ticker_plugin>(); }
...
static const plugin_descriptor g_ticker_desc{
	"ticker",
	std::span<araya::dependency_spec const>(g_ticker_deps),
	g_no_provs,
	&make_ticker};
...
struct app_plugin : araya::plugin {
	...
	task<void> apply(araya::plugin_context& ctx) override {
		for (int i = 0; i < children; ++i)
			co_await ctx.mount(
				component_spec{shared_desc(g_ticker_desc), {}, nullptr, "ticker." + std::to_string(i), {}});
		co_return;
	}
};
...
static const plugin_descriptor g_app_desc{"app", g_no_deps, g_no_provs, &make_app};
...
task<result> workload_cascade(runtime& rt, long long iterations, int depth) {
	chain_set set(depth);
	auto t0 = clock_type::now();
	for (long long it = 0; it < iterations; ++it) {
		auto hd = co_await rt.mount(component_spec{shared_desc(set.head), {}, nullptr, "head", {}});
		std::vector<fiber_handle> mids(depth);
		for (int i = depth - 1; i >= 0; --i)
			mids[i] = co_await rt.mount(
				component_spec{shared_desc(set.descs[i]), {}, nullptr, "chain." + std::to_string(i), {}});
		auto tl = co_await rt.mount(component_spec{shared_desc(set.tail), {}, nullptr, "tail", {}});
		co_await rt.wait_idle();
	...
	double wall = ms_since(t0);
	co_return result{"cascade", iterations, wall, rt.fiber_count()};
}
...
task<result> workload_replace(runtime& rt, long long iterations) {
	...
	co_await rt.retire(cur);
	co_await rt.wait_idle();
	co_return result{"replace", iterations, wall, rt.fiber_count()};
}
...
task<result> workload_reconcile(runtime& rt, long long iterations) {
	...
	for (long long i = 0; i < iterations; ++i) {
		...
		if (!has_p && !has_s)
			has_s = true;
		if (has_p)
			desired.push_back({"p", {shared_desc(g_desc_P), {}, nullptr, "P", {}}});
		if (has_o)
			desired.push_back({"o", {shared_desc(g_desc_O), {}, nullptr, "O", {}}});
		if (has_s)
			desired.push_back({"s", {shared_desc(g_desc_S), {}, nullptr, "S", {}}});
	...
	co_await rt.wait_idle();
	double wall = ms_since(t0);
	co_return result{"reconcile", iterations, wall, rt.fiber_count()};
}
...
task<result> workload_steady(runtime& rt, int children, long long steady_ms, araya::timer::timer_service& timer) {
	g_ticks.store(0);
	plugin_config cfg{{"children", std::to_string(children)}};
	auto app = co_await rt.mount(component_spec{shared_desc(g_app_desc), std::move(cfg), nullptr, "app", {}});
	co_await rt.wait_idle();
	auto t0 = clock_type::now();
	...
	co_await rt.wait_idle();
	co_return result{"steady", g_ticks.load(), wall, rt.fiber_count()};
}
...
task<void> bench_main(
	runtime& rt,
	std::string const& workload_name,
	...
	auto log = rt.root_context().require<araya::logger::logger_service>(araya::logger::logger_key);
	auto timer = rt.root_context().require<araya::timer::timer_service>(araya::timer::timer_key);

	std::vector<std::string> workloads;
	if (workload_name == "all")
		workloads = {"mount", "cascade", "replace", "reconcile", "steady"};
	else
	...
	for (auto const& w : workloads) {
		...
		if (w == "mount")
			...
			r = co_await workload_steady(rt, components, steady_ms, *timer);
		else {
...
options parse_args(int argc, char** argv) {
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (arg == "--workload")
			...
			o.iterations = arg_value<long long>(next("--iterations"), "--iterations");
		else if (arg == "--components")
			o.components = arg_value<int>(next("--components"), "--components");
		else if (arg == "--depth")
			o.depth = std::max(1, arg_value<int>(next("--depth"), "--depth"));
		else if (arg == "--steady-ms")
			o.steady_ms = arg_value<long long>(next("--steady-ms"), "--steady-ms");
...
int main(int argc, char** argv) {
	...
	component_spec logger_spec{
		shared_desc(araya::logger::plugin_descriptor()), std::move(log_cfg), nullptr, "logger", {}};
	component_spec timer_spec{shared_desc(araya::timer::plugin_descriptor()), {}, nullptr, "timer", {}};
